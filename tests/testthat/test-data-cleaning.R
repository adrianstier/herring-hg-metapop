...
test_that(".build_stan_input maps version-specific fields correctly", {
  ...
  expect_true(3L %in% v1_input$q_idx)
  ...
  expect_equal(length(m5_input$whale_obs), N_YEARS)
  expect_equal(length(m5_input$pred_obs), N_YEARS)
  expect_equal(dim(m6_input$seal), c(N_YEARS, N_SITES))
  expect_equal(dim(m6_input$ssl), c(N_YEARS, N_SITES))
  expect_equal(m6_input$ssl, model_spatial$ssl_spatial)
  expect_equal(m6_input$seal, model_spatial$seal_spatial)
