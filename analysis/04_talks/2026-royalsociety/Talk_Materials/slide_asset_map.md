...
## Act I — Cold open (slides 1–4, incl. S1b sense-of-place + S3.5 wasp-waist)
...
| 1b | **Sense of place — *Haida Gwaii at the edge of the North Pacific*** | ✅ `deck_assets/01b_sense_of_place.png` (baked from `s01b_haida_gwaii_place.html`; ESRI World Imagery / Maxar / Sentinel-2 satellite mosaic of the full archipelago; deck-styled annotations: shelf edge, upwelling, protected spawning coves; amber dots for **Skidegate / Cumshewa / Selwyn** — the inlets cited later) | `deck_build/photos/s_locator_l2.png` (former locator — globe→NA→HG inset; demoted backup) | ✅⚠️ | BUILT — replaces the EEMB142C teaching-deck locator (s_locator_l2.png, lines 60–75 of `build_pptx_native.js` pre-2026-05-20). Real satellite imagery + functional annotation labels for an audience that has never been there. **Attribution required:** "Imagery: Esri World Imagery (Maxar / Sentinel-2 mosaic)" — baked into the slide footer. Source raster: `deck_assets/_sources/haida_gwaii_satellite_esri.jpg` (2400×2800, bbox -133.4..-131.0 lon, 51.6..54.4 lat). ~25 s beat |
