...
readiness <- tribble(
  ...
  "herring_age_composition", "herring", "year x SAR x fleet/source x age", "age-composition likelihood", "published_extractable",
  paste(bio_manifest_summary, bio_file_summary),
  "DFO input manifest and public assessment reports reference number-at-age/proportion-at-age tables, but model-ready annual age tables are not packaged locally.",
  "Extract public Appendix B age-composition tables first; request exact SCA/SISCAH input CSVs only if PDF/table extraction cannot reproduce the assessment inputs.",
  "herring_weight_at_age", "herring", "year x SAR x age", "spawning biomass and catch-at-age conversion", "published_extractable",
  paste(bio_manifest_summary, bio_file_summary),
  "DFO input manifest and public assessment reports reference weight-at-age matrices, but model-ready annual matrices are not packaged locally.",
  "Extract public Appendix B weight-at-age tables first; request exact SCA/SISCAH input CSVs and sample-size metadata if extraction is lossy.",
  ...
  "herring_maturity_at_age", "herring", "age", "mature biomass and spawning selectivity", "published_extractable",
  "DFO assessment documentation gives the fixed maturity schedule: age 2 about 25%, age 3 about 90%, age 4+ mature.",
  "Schedule is published but not yet encoded locally with source metadata.",
  "Create a sourced maturity table after confirming the schedule from current DFO assessment inputs.",
...
registry_md <- source_registry %>%
  select(dataset_id, required_product, status, acquisition_method, primary_source, source_url, notes)
...
lines <- c(
  ...
  "- It does **not** yet have the herring biological inputs needed for a true catch-at-age model in model-ready local form: raw age composition, weight-at-age, length-at-age, and test-fishery biological tables are absent or not model-ready.",
  "- The DFO stock-assessment page and assessment appendices are the first acquisition path: extract the published HG catch, spawn, number-at-age, weight-at-age, and sample-size tables before sending a narrower DFO request for exact SCA/SISCAH input files.",
