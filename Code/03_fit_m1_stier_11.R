# ============================================================================
# 03_fit_m1_stier_11.R
# Fit Stier-aligned M1 baseline with 11 sections and ambiguous zeros.
#
# Key choices:
...
#   - current DFO tonnes-scale spawn index is used first; do not copy legacy SHI
#     q posteriors onto this scale.
# ============================================================================
...
cat("\n", strrep("=", 60), "\n")
cat(" MODEL M1_STIER_11: positive-only zeros-ambiguous baseline\n")
cat(strrep("=", 60), "\n\n")
...
saveRDS(fit, file.path(data_dir, "m1_stier_11_fit.rds"))
saveRDS(fit, file.path(out_dir, "fit_m1_stier_11.rds"))

log_lik <- rstan::extract(fit, "log_lik", permuted = FALSE)
r_eff <- relative_eff(exp(log_lik))
loo_m1 <- loo(log_lik, r_eff = r_eff)
saveRDS(loo_m1, file.path(out_dir, "loo_m1_stier_11.rds"))

summ <- summarize_draws(fit)
write.csv(
  summ,
  file.path(proj_dir, "Output", "m1_stier_11_parameter_summary.csv"),
  row.names = FALSE
)

cat("\n=== M1_STIER_11 COMPLETE ===\n")
