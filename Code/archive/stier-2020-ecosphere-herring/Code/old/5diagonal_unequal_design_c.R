...
for(i in 2:nYears){
 ...
 ",file="diagonal_unequal_design_c.txt")
...
jags.params=c("X","sigma2.all","U","Umu","Usig2","delta","Z","pdocoef","log.q","Pc","tauR2")


model.loc="diagonal_unequal_design_c.txt" # name of the txt file
...
#      "design.c","inits",
#      file="diag_unequal_design_c.RData")
setwd("~/Dropbox/Projects/In Progress/Herring_Haida_Gwaii/Code")
...
load("diag_unequal_design_c.RData")


## ==================
## Use previous run mean posterior as starting values for new run
## ==================
...
for(i in 1:n.chains){
  inits[[i]]    <- list(
    "Umu" = model$BUGSoutput$mean$Umu,
    "Usig2" = model$BUGSoutput$mean$Usig2,
    "pdocoef" = model$BUGSoutput$mean$pdocoef,
    "log.q" = model$BUGSoutput$mean$log.q,
    "tauR2" = 1,
    "sigma2.all" = model$BUGSoutput$mean$sigma2.all,
    "U" = model$BUGSoutput$mean$U
...
pdf(paste("diagonal_unequal_design_c",Sys.Date(),".pdf"), onefile = TRUE)
...
slist<-list(qdf,tauRdf,pdodf,Umudf,Usigdf)
nm<-c("log.q","tauR","pdocoef","Umu","Usig2")
