...
#VARINCE MATRIX - DIAGONAL & UNEQUAL
...
for(i in 2:nYears){
  ...
  for(j in 1:nSites){
    ...
    Z[i,j] <- X[(i-1),j]+U[j]+pdocoef*pdo[i-1]+delta[(i-1),j] #fishing by site and global pdo estimates and U estimates   
 ...
 ",file="diagonal_unequal_design_c_DDspatial.txt")
...
jags.params=c("X","sigma2.all","U","Umu","Usig2","delta","Z",
			"pdocoef","log.q","Pc","tauR2","Q","theta")



model.loc="diagonal_unequal_design_c_DDspatial.txt" # name of the txt file

#going from 5000 iterations to 50000 iterations led to this error message:
#Non positive definite matrix in call to logdet
...
save("model","mDIC","inits",
     file="diagonal_unequal_design_c_DDspatial.Rdata")

setwd("~/Dropbox/Projects/In Progress/Herring_Haida_Gwaii/Code")

load("diagonal_unequal_design_c_DDspatial.Rdata")
...
pdf(paste("diagonal_unequal_design_c_DDspatial",Sys.Date(),".pdf"), onefile = TRUE)
...
slist<-list(qdf,tauRdf,pdodf,Umudf,Usigdf,thetadf)
nm<-c("q","tauR2","pdocoef","Umu","Usig2","theta")
