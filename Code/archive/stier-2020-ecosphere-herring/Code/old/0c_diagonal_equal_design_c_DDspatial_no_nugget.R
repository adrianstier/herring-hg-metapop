...
for(i in 1:nSites) {
	for(j in 1:nSites) {
    	Q[i,j] <- sigma.allb[i]*sigma.allb[j] * exp(-distMat5[i,j]/theta)
...
setwd("~/Dropbox/Projects/In Progress/Herring_Haida_Gwaii/Code")
load("diagonal_equal_design_c_DDspatial_noUsig.Rdata")
...
# setwd("~/Desktop/Herring_2016")
pdf(paste("diagonal_equal_design_c_DDspatial_noUsig_no_nugget",Sys.Date(),".pdf"), onefile = TRUE)
...
## ==================
##  Predicted SHI and Observed SHI
## ==================
...
## ==================
##  Predicted Scaled Spawning Biomass 
## ==================
...
slist<-list(qdf,tauRdf,pdodf,Umudf,thetadf,sigma2df)
nm<-c("log.q","tauR2","pdocoef","Umu","theta","sigma2")
...
# ==================
#   Predicted Catch Through Time At Different Sites 
# ==================
