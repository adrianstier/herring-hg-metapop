...
for(i in 1:nSites) {
	for(j in 1:nSites) {
    	Q[i,j] <- sigma2.allb[i]*sigma2.allb[j] * exp(-distMat5[i,j]/theta)
  }
}   
...
delta[1,1:nSites] ~ dmnorm(zeros,tauQ[1:nSites,1:nSites]); #check this. it's the vcov matrix and maybe should involve rho in the vcov term. 
...
for(i in 2:nYears){
 ...
 ",file="diagonal_equal_design_c_DDspatial_autocor_delta_noUsig.txt")
...
model.loc="diagonal_equal_design_c_DDspatial_autocor_delta_noUsig.txt" # name of the txt file
...
save("model","mDIC","inits",
     file="diagonal_equal_design_c_DDspatial_autocor_delta_noUsig_imac.Rdata")

setwd("~/Dropbox/Projects/In Progress/Herring_Haida_Gwaii/Code")

load("diagonal_equal_design_c_DDspatial_autocor_delta_noUsig.Rdata")
...
pdf(paste("diagonal_equal_design_c_DDspatial_autocor_delta_noUsig",Sys.Date(),".pdf"), onefile = TRUE)
...
slist<-list(qdf,pdodf,Umudf,tauRdf,thetadf,rhodf,sigma2df)
nm<-c("q","pdocoef","Umu","tauR2","theta","rho","sigma2")
