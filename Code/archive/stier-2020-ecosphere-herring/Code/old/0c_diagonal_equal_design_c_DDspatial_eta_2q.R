...
for(i in 2:nYears){
 ...
 ",file="diagonal_equal_design_c_DDspatial_noUsig_2q.txt")
...
jags.data = list("Y"=logSHI, "nYears"=nYears,"nSites"=nSites,"pdo"=pdo2,"ctab"= logcatch,
                 ...
                 "distMat5"=distMat5,"Diag"=diag(nSites),"q_idx"=q_idx) # named list
...
model.loc="diagonal_equal_design_c_DDspatial_noUsig_2q.txt" # name of the txt file
...
save("model","mDIC","inits","design.c",
     file="diagonal_equal_design_c_DDspatial_noUsig_eta_thetaprior_2q.Rdata")

setwd("~/Dropbox/Projects/In Progress/Herring_Haida_Gwaii/Code")

load("diagonal_equal_design_c_DDspatial_noUsig_eta_thetaprior_2q.Rdata")
...
# setwd("~/Desktop/Herring_2016")
pdf(paste("diagonal_equal_design_c_DDspatial_noUsig_nugget_thetaprior_2q",Sys.Date(),".pdf"), onefile = TRUE)
...
slist<-list(qdf1,qdf2,tauRdf,pdodf,Umudf,sigma2df,thetadf,etadf)
nm<-c("log.q[1]","log.q[2]","tauR2","pdocoef","Umu","sigma2","theta","eta")
