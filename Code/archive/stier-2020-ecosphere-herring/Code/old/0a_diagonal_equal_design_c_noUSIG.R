...
setwd("~/Dropbox/Projects/In Progress/Adrian/Herring stocklet dynamics/Code")
source('multiplot.R')
source('theme_acs.R')
...
for(i in 2:nYears){
 ...
 ",file="diag_equal_design_c_noUsig.RData.txt")
...
model.loc="diag_equal_design_c_noUsig.RData.txt" # name of the txt file
...
save("model","mDIC","inits",
     file="diag_equal_design_c_noUsig.RData")



setwd("~/Dropbox/Projects/In Progress/Herring_Haida_Gwaii/Code")

load("diag_equal_design_c_noUsig.RData")
...
pdf(paste("diagonal_equal_design_c_noUsig",Sys.Date(),".pdf"), onefile = TRUE)
...
slist<-list(qdf,tauRdf,pdodf,Umudf,sigma2df)
nm<-c("log.q","tauR2","pdocoef","Umu","sigma2")
