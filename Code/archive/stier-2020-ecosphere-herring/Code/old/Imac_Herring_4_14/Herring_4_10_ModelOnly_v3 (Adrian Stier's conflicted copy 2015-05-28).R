...
for(j in 1:nSites) {
      X[1,j] ~ dnorm(20,1/zeta2); # vague normal prior for first time step
...
save("model","X","theta","sigma2","U","Umu","Usig","delta","Q","pdo","pdocoef","Pc","q","tauR",file="herring_jags_F_9sites.RData")
...
for(i in 1:length(jags.params)){
  temp<-model$BUGSoutput$sims.array[,,jags.params[i]]
  
  temp<-model$BUGSoutput$sims.array[,,"pdocoef"]
...
#add site names from df2
ggplot(temp2,aes(x=time,y=factor(site),fill=Pc))+
  geom_tile()+
  theme_acs()
