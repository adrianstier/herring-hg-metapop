...
for(i in 2:nYears){
 ...
 ",file="diagonal_equal_design_c_autocor_delta.txt")
...
jags.params=c("X","sigma2","U","Umu","Usig2","delta","Z","pdocoef","log.q","Pc","tauR2","rho")


model.loc="diagonal_equal_design_c_autocor_delta.txt" # name of the txt file
...
load("diag_equal_design_c_autocor_delta.RData")
...
pdf(paste("diagonal_equal_design_c_autocor_delta",Sys.Date(),".pdf"), onefile = TRUE)
...
slist<-list(qdf,tauRdf,pdodf,Umudf,Usigdf,sigma2df,rhodf)
nm<-c("log.q","tauR2","pdocoef","Umu","Usig2","sigma2","rho")
