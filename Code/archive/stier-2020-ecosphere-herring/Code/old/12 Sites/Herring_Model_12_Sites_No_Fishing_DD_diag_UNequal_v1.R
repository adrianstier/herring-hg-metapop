...
X[i,j] <- X[i-1,j]+U[j]+pdocoef*pdo[i-1]+delta[i,j]
 ...
 ",file="normal_spatialRW_noF_diag_uneq_12sites.txt")


#data going into the model
jags.data = list("Y"=Y, "nYears"=nYears,"nSites"=nSites,"distMat5"=distMat5,"pdo"=pdo3) # named list
jags.data.p = list("Y", "nYears","nSites","distMat5","pdo","ctab","c_prior") # named list



jags.params=c("X","theta","sigma2","U","Umu","Usig","delta","Q","Z","pdocoef","q","tauR") # parameters in the linear model


model.loc="normal_spatialRW_noF_diag_uneq_12sites.txt" # name of the txt file
...
save("model","mDIC","X","theta","sigma2","U","Umu","Usig","Z","delta","Q","pdo","pdocoef","Pc","q","tauR",file="herring_jags_NO_F_12sites_DD_unequal_var.RData")

#load("herring_jags_NO_F_12sites_equal_var.RData")
########################################################################
