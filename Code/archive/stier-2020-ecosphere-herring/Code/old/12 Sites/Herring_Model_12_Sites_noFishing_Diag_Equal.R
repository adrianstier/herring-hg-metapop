...
Q[1] ~ dgamma(0.001,0.001); #set initial

  for(i in 2:nSites) {
    Q[i] <- Q[1]; #replicate initial value for all Q
}
...
for(j in 1:nSites) {
      X[1,j] ~ dnorm(10,inverse(Q[j])); # vague normal prior for first time step
}


#  Delta Values
for(i in 1:nYears){
for(j in 1:nSites) {delta[i,j] ~ dnorm(0,inverse(Q[j]))
}
}
...
X[i,j] <- X[i-1,j]+U[j]+pdocoef*pdo[i-1]+delta[i,j]
}
}
 
} 
 ",file="12sites_NoFishing_DiagonalEqual.txt")


#data going into the model
jags.data = list("Y"=Y, "nYears"=nYears,"nSites"=nSites,"pdo"=pdo3) # named list
jags.data.p = list("Y", "nYears","nSites","distMat5","pdo") # named list



jags.params=c("X","U","Umu","Usig","delta","Q","pdocoef","q","tauR") # parameters in the linear model


model.loc="12sites_NoFishing_DiagonalEqual.txt" # name of the txt file
...
save("model","mDIC","X","U","Umu","Usig","delta","Q","pdo","pdocoef","q","tauR",file="herring_jags_12sites_NoFishing_DiagonalEqual.RData")

load('herring_jags_12sites_NoFishing_DiagonalEqual.RData')
...
pdf(paste("12sites_NoFishing_DiagonalEqual_Chains_Posteriors",Sys.Date(),".pdf"), onefile = TRUE)
...
ucomp<-ggplot(udf,aes(x=Var3,y=value,colour=Var3))+
  ...
  ggtitle("Populaiton Growth -diagonal and equal")
...
udf #u tab
qlist<-model$BUGSoutput$sims.array[,,colnames(myList[[1]])[which(colnames(myList[[1]])=="Q[1]"):which(colnames(myList[[1]])=="Q[12]")]]
dimnames(qlist)[[3]]<-sitelab
qdf<-melt(qlist)

sl<-unique(udf$Var3)

emat<-matrix(NA,nSites,6)
for(i in 1:nSites){
  temp<-drop.levels(subset(udf,Var3==sl[i]))
  emat[i,1] = smedian.hilow(temp$value,conf.int=0.5)[1]
  emat[i,2] = smedian.hilow(temp$value,conf.int=0.5)[2]
  emat[i,3] = smedian.hilow(temp$value,conf.int=0.5)[3]
  
  temp2<-drop.levels(subset(qdf,Var3==sl[i]))
  emat[i,4] = smedian.hilow(temp2$value,conf.int=0.5)[1]
  emat[i,5] = smedian.hilow(temp2$value,conf.int=0.5)[2]
  emat[i,6] = smedian.hilow(temp2$value,conf.int=0.5)[3]    
}

mvdf<-data.frame(emat)
names(mvdf)<-c("medU","lowU","HiU","medQ","lowQ","HiQ")
mvdf$section<-sl

ggplot(mvdf,aes(x=medU,y=medQ))+
  geom_pointrange(aes(ymin = lowQ, ymax = HiQ)) +
  geom_errorbarh(aes(xmax = HiU, xmin = lowU, height = 0))+
  geom_point(aes(colour=section),size=3)+
  geom_smooth(se=F)+
  xlim(min(mvdf$lowU),max(mvdf$HiU))+
  ylim(min(mvdf$lowQ),max(mvdf$HiQ))+
  theme_acs()+
  ggtitle("Mean-Variance of Population Growth")
...
nm<-c("q","pdocoef","theta","Umu","Usig","tauR")
...
model$BUGSoutput$sims.array[,,"Q[1]"]

edf1<-melt(model$BUGSoutput$sims.array[,,colnames(myList[[1]])[which(colnames(myList[[1]])=="Q[1]"):which(colnames(myList[[1]])=="Q[12]")]])
names(edf1)<-c("num","chain","response","value")
edf1$chain<-factor(edf1$chain)
