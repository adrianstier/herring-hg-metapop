...
theta ~ dunif(0.01,0.99) #theta ~ dgamma(1.2,0.6)
...
X[i,j] <- X[i-1,j]+U[j]+pdocoef*pdo[i-1]+delta[i,j]
 ...
 ",file="12sites_NoFishing_DistanceDecay.txt")
...
jags.params=c("X","theta","sigma2","U","Umu","Usig","delta","Q","pdocoef","q","tauR") # parameters in the linear model


model.loc="12sites_NoFishing_DistanceDecay.txt" # name of the txt file
...
save("model","mDIC","X","theta","sigma2","U","Umu","Usig","delta","Q","pdo","pdocoef","q","tauR",file="herring_jags_12sites_NoFishing_Distance_Decay.RData")

load("herring_jags_12sites_NoFishing_Distance_Decay.RData")
########################################################################
...
pdf(paste("12sites_NoFishing_DistanceDecay_Chains_Posteriors",Sys.Date(),".pdf"), onefile = TRUE)
