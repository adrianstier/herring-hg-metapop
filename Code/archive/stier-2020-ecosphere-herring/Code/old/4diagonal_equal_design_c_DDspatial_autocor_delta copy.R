...
for(i in 2:nYears){
 ...
 ",file="diagonal_equal_design_c_DDspatial_autocor_delta.txt")
...
jags.params=c("X","sigma2","U","Umu","Usig2","delta","Z",
			"pdocoef","log.q","Pc","tauR2","Q","theta","rho")



model.loc="diagonal_equal_design_c_DDspatial_autocor_delta.txt" # name of the txt file
...
save("model","mDIC","inits",
     file="diagonal_equal_design_c_DDspatial_autocor_delta.Rdata")

setwd("~/Dropbox/Projects/In Progress/Herring_Haida_Gwaii/Code")

load("diagonal_equal_design_c_DDspatial_autocor_delta.Rdata")
...
pdf(paste("diagonal_equal_design_c_DDspatial_autocor_delta",Sys.Date(),".pdf"), onefile = TRUE)
...
slist<-list(qdf,pdodf,Umudf,Usigdf,tauRdf,thetadf,rhodf,sigma2df)
nm<-c("q","tauR2","pdocoef","Umu","Usig2","theta","rho","sigma2")
