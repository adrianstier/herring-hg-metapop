...
u_idx <- c(rep(1,1),rep(2,10),rep(3,10),rep(4,10),rep(5,10),rep(6,10),rep(7,5),rep(8,5),rep(9,5))
...
jagsscript = cat("
                 ...
                 zeta2 ~ dgamma(1,100) ### go to dgamma(0.1,10)
                  ...
                  for(i in 2:9) {
                    Umu[i] ~ dnorm( rho * Umu[i-1], 1 / (zeta2 / ( 1- rho^2)) ) 
                  }
                 ...
                 for(i in 2:nYears){
                    for(j in 1:nSites) {
                      ...
                      Z[i,j] <- X[(i-1),j]+ Umu[u_idx[i]] + pdocoef*pdo[i-1] + delta[(i-1),j] #edited this to ref idx of u
                 ...
                 ",file="diag_equal_design_c_2q_timevaryingU_ubins.RData.txt")


#data going into the model
jags.data = list("Y"=logSHI, "nYears"=nYears,"nSites"=nSites,"pdo"=pdo2,"ctab"=logcatch,
                 "INDEX"=INDEX,"INDEX.zero"=INDEX.zero, "nIndex"=nIndex,"nIndex.zero"=nIndex.zero,"q_idx"=q_idx,"u_idx"=u_idx) # named list

jags.params=c("X","sigma2","Umu","delta","Z","pdocoef","log.q","Pc","tauR2","rho","zeta2")

model.loc="diag_equal_design_c_2q_timevaryingU_ubins.RData.txt" # name of the txt file
...
save("model","mDIC","inits",
     file="diag_equal_design_c_noUsig_2q_timevaryingU_ubins.RData")

load("diag_equal_design_c_noUsig_2q_timevaryingU_ubins.RData") 
