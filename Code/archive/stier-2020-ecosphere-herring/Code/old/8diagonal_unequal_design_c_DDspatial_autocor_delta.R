...
#dummy variabl for q matrix

for(j in 1:nSites) {
 	sigma2.allb[j] <-sqrt(sigma2.all[j])
}
...
for(i in 1:nSites) {
	for(j in 1:nSites) {
    	Q[i,j] <- sigma2.allb[i]*sigma2.allb[j] * exp(-distMat5[i,j]/theta) #no nugget 
  }
}   
...
for(i in 2:nYears) {
      ...
      delta[i,1:nSites] ~ dmnorm(rho*delta[i-1,1:nSites],tauQ[1:nSites,1:nSites]); #autoregressive
	    
      #delta[i,1:nSites] ~ dmnorm(zeros,tauQ[1:nSites,1:nSites]); #no autocor
 ...
 ",file="diagonal_unequal_design_c_DDspatial_autocor_delta.txt")
...
jags.data = list("Y"=logSHI, "nYears"=nYears,"nSites"=nSites,"pdo"=pdo2,"ctab"= logcatch,
                 ...
                 "distMat5"=distMat5) # named list

jags.params=c("X","sigma2.all","U","Umu","Usig2","delta","Z",
			"pdocoef","log.q","Pc","tauR2","Q","theta","rho")


model.loc="diagonal_unequal_design_c_DDspatial_autocor_delta.txt" # name of the txt file
...
## ==================
#save("model","mDIC","X","U","Umu","Usig2","Z","delta","sigma2.all","pdo","pdocoef","Pc","log.q","tauR2",
#     "design.c","theta",
#     file="diagonal_unequal_design_c_DDspatial_autocor_delta.Rdata")
...
load("diagonal_unequal_design_c_DDspatial_autocor_delta.Rdata")
...
pdf(paste("diagonal_unequal_design_c_DDspatial_autocor_delta",Sys.Date(),".pdf"), onefile = TRUE)
...
#sigma2df<-melt(model$BUGSoutput$sims.array[,,"sigma2.all"])

slist<-list(qdf,tauRdf,pdodf,Umudf,Usigdf,thetadf,rhodf)
nm<-c("q","tauR2","pdocoef","Umu","Usig2","theta","rho")
...
gtemp2<-ggplot(sdf,aes(x=value))+
  ...
  facet_wrap(~section2,scales="free_x")+
