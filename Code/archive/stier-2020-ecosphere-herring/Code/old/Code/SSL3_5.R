...
#site map
al1 = get_map(location = c(-135,51.5,-131,55), zoom = 3, maptype = 'terrain')
al1MAP = ggmap(al1)


tn<-x[,c(1:4,7,19)]
tn2<-melt(tn,names(tn)[1:4])
tn2$presabs<-as.numeric((tn2$value > 0) & (!is.na(tn2$value)))
tn2$SiteType2 <- ifelse(tn2$SiteType=="R","Rookery","Haulout")


sitesSSL<-al1MAP+
  geom_point(data = tn2, aes(x = Longitude, y = Latitude,fill=factor(presabs),size=value),pch=21)+
  geom_point(data = tn2, aes(x = Longitude, y = Latitude,pch=SiteType2))+
  #geom_text(data = tn2,aes(x = Longitude, y = Latitude,label=Site),vjust=.1,hjust=-.1)+
  scale_size(range = c(4, 10))+
  facet_wrap(~variable,ncol=2)+
  theme_acs()

ggsave("sitesSSL1971_2013.pdf",width=10,height=8)
...
#site map
al1 = get_map(location = c(-135,51.5,-131,55), zoom = 1, maptype = 'terrain')
al1MAP = ggmap(al1)
...
multiplot(ggtemp71,ggtemp13,cols=2)

####################################
#####Just Haul Out Sites SIZE converted to herring demand
####################################

x4b$consumption<-300*(x4b$count*6)/1000 #demand in tons per year double check it's 1000 may be 2000 kg in metric ton

consum<-melt(tapply(x4b$consumption,list(x4b$year),sum,na.rm=T))
ggcons<-ggplot(consum,aes(x=Var1,y=value))+
  geom_line()+
  ylab("consumption by sea lions in tons")+
  xlab("year")+
  theme_acs()
...
for(i in 1:length(yr)){
  temp <- subset(x4b,year==yr[i]&presabs==1)
  ggtemp<-ggmap(al1)+
    geom_point(data = temp, aes(x = Longitude, y = Latitude,size=consumption))+
    scale_size_continuous(limits=c(0,800),range=c(4,10))+
    theme_acs()+
    ggtitle(paste(yr[i],"_SSL"))
  print(ggtemp)
  
  ggsave(paste(yr[i],"_SSL_HauloutOnly_herringdemand.png"))
}
