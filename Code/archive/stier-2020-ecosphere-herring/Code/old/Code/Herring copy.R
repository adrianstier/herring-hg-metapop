...
theta_df<-data.frame(theta,chain,num)

ggplot(theta_df,aes(y=theta,x=num,group=chain))+
...
#Umu - Average trend for all population
Umu_df<-data.frame(Umu)
ggplot(Umu_df,aes(x=Umu))+
  geom_bar()+
  theme_acs()
