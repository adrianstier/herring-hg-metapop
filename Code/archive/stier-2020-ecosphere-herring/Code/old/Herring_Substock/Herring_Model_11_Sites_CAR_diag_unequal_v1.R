...
for(i in 1:nSites) {
  tau2Inv_t[i,i] <- 10#rgamma(1,0.01,0.01); #this is for sd in cov matrix
}

for(i in 1:nSites){
  tau2Inv_t[i,i] <- (tau2Inv_t[i,i])^-1
}
...
for(i in 1:nSites) {
  tau2Inv[i,i] ~ dgamma(0.01,0.01); #this is for sd in cov matrix
}


for(i in 1:nSites){
  tau2Inv2[i,i] <- (tau2Inv[i,i])^-1
}


Q <- (tau2Inv2 %*% inverse(IDEN - phi * H))
tauQ <- inverse(Q);
...
for(i in 2:nYears) {
 ...
 ",file="normal_spatialRW_11sites_CAR_unequalvar.txt")
...
model.loc="normal_spatialRW_11sites_CAR_unequalvar.txt" # name of the txt file
...
save("model","mDIC","X","U","Umu","Usig","Z","delta","Q","pdo","pdocoef","Pc","q","tauR","tau2Inv",file="herring_jags_F_11sites_UNequal_var_CAR.RData")

#load("herring_jags_F_11sites_UNequal_var_CAR.RData")
########################################################################
...
pdf(paste("12sites_CAR_UNequalvar_Chains_Posteriors",Sys.Date(),".pdf"), onefile = TRUE)
