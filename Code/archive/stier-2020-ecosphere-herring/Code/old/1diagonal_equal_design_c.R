...
Usig2 ~  dnorm(1,1) #variance in population growth 
...
for(i in 2:nYears){
 ...
 ",file="diagonal_equal_design_c.txt")
...
jags.params=c("X","sigma2","U","Umu","Usig2","delta","Z","pdocoef","log.q","Pc","tauR2")


model.loc="diagonal_equal_design_c.txt" # name of the txt file
...
#save("model","mDIC","X","sigma2","U","Umu","Usig2","Z","delta","sigma2","pd##","pdocoef","Pc","log.q","tauR2",
  #   "design.c","inits",
   #  file="diag_equal_design_c.RData")

setwd("~/Dropbox/Projects/In Progress/Herring_Haida_Gwaii/Code")

save("model","mDIC","inits",
     file="diag_equal_design_c_hardUsigPrior.RData")



setwd("~/Dropbox/Projects/In Progress/Herring_Haida_Gwaii/Code")

load("diag_equal_design_c.RData")


## ==================
## Take previous run and use as starting/initial values
## ==================
...
for(i in 1:n.chains){
  inits[[i]]    <- list(
    "Umu" = mean(Umudf[(runL-100):runL,]$value),
    "Usig2" = mean(Usigdf[(runL-100):runL,]$value),
    "pdocoef" = mean(pdodf[(runL-100):runL,]$value),
    "log.q" = mean(qdf[(runL-100):runL,]$value),
    "tauR2" = mean(tauRdf[(runL-100):runL,]$value),
   	"sigma2" = mean(sigma2df[(runL-100):runL,]$value)
    ) 
}
...
pdf(paste("diagonal_equal_design_c_Usigprior",Sys.Date(),".pdf"), onefile = TRUE)
...
slist<-list(qdf,tauRdf,pdodf,Umudf,Usigdf,sigma2df)
nm<-c("log.q","tauR2","pdocoef","Umu","Usig2","sigma2")
