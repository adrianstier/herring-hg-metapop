...
for(j in 1:nSites){
  ...
  zmat[1,j]<-rnorm(1,aveSHI[j],1) #assume pre catch biomass (Z) is distriubted SHI 
...
log.q ~ dnorm(0,(1/.5));  #Catchability Conversion from Spawn Index to Spawn Biomass 
tauR ~ dunif(0,100); #this is the estimated variance prior for Spawn Index to Spawn Biomass Conversion
psi ~ dunif(0,100) #this is in variance for fishing set fixed to try to get convergence


for(i in 1:nYears) {
  for(j in 1:nSites) {
                 Pc[i,j] ~ dunif(0,0.95) #Fraction of Spawn Caught for each site
  }
}
...
for(i in 1:nYears) {
  for(j in 1:nSites) {
    Y[i,j] ~  dnorm((X[i,j]+log.q),1/tauR); #obs eq for spawn index. a normal pull with meam Xq LIKELIHOOD STATEMENT ****
    
    tl[i,j]<-Z[i,j]+log(Pc[i,j])*design.c[i,j] #total biomass caught (tl) is qual to to estimated biomass (Z) pre catch + catch (Pc) 
...
sigma2[1] ~ dgamma(0.001,0.001); #set initial
  ...
  for(j in 2:nSites) {
    sigma2[j] <- sigma2[1]; #replicate initial value for all sigma2
}
...
for(j in 1:nSites) {
      ...
      X[1,j] <- Z[1,j] + log(1-Pc[1,j])*design.c[1,j] # changed to reflect handwritten model 3/31
}
...
X[i,j] <-Z[i,j]+log(1-Pc[i,j])*design.c[i,j];

 }
} 
} 
 ",file="simulator_diagonal_equal_design.c.txt")


#data going into the model
jags.data = list("Y"=SHI_sim, "nYears"=nYears,"nSites"=nSites,"pdo"=pdo3,"ctab"=catch_sim,"design.c"=design.c) # named list

jags.params=c("X","sigma2","U","Umu","Usig2","delta","Z","pdocoef","log.q","Pc","tauR","psi") # parameters in the linear model


model.loc="simulator_diagonal_equal_design.c.txt" # name of the txt file
...
for(i in 1:n.chains){
  inits[[i]]    <- list(
    "Umu" = rnorm(1,0,1), #alt just make it a little lower? 0.5 seems more realisitc
    "Usig2" = runif(1,.05,1),
    "pdocoef" = rnorm(1,0,2),
...
save("model","mDIC","X","sigma2","U","Umu","Usig2","Z","delta","sigma2","pdo","pdocoef","Pc","q","tauR","psi",
     "q_sim","psi_sim","pdocoef_sim","Umu_sim","Usig_sim","tauR_sim","sigma2_sim","design.c",
...
nm<-c("logq","psi","pdocoef","Umu","Usig","tauR","sigma2")
