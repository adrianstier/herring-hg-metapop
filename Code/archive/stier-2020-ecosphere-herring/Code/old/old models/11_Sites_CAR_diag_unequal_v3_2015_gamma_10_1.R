...
tauR ~ dunif(0,100); #this is the estimated variance prior for Spawn Index on a log scale
...
for(i in 1:nYears) {
  for(j in 1:nSites) {
    ...
    tl[i,j]<-Z[i,j]+log(Pc[i,j]) #total catch (tl) is on the log scale equal to the log of the pre catch biomass (z) + log of the catch fraction 
...
for(i in 2:nYears) {
 ...
 ",file="normal_spatialRW_11sites_CAR_unequalvar_2015_psi_dunif_0_10.txt")
...
jags.params=c("X","U","Umu","Usig","tau2","phi","delta","Qinv","Z","pdocoef","q","Pc","tauR","psi") # parameters in the linear model


model.loc="normal_spatialRW_11sites_CAR_unequalvar_2015_psi_dunif_0_10.txt" # name of the txt file
...
for(i in 1:n.chains){
  inits[[i]]    <- list(
    ...
    "tauR" = runif(1,1,2),
    "Pc" = matrix(1e-05,nrow=nYears,ncol=nSites)
#     , #runif(1,.05,1) #reducing this and the variance on this psi
...
save("model","mDIC","X","U","Umu","Usig","Z","delta","Qinv","phi","pdo","pdocoef","Pc","q","tauR","tau2","psi",file="herring_jags_F_11sites_UNequal_var_CAR_2015_v6_psi_duif_0_10.RData")

load("herring_jags_F_11sites_UNequal_var_CAR_2015_v6_psi_duif_0_10.RData")
...
#head(model$BUGSoutput$sims.array)
  ...
  pdf(paste("11sites_CAR_UNequalvar_Chains_Posteriors_1950_2015_psi_dunif",Sys.Date(),".pdf"), width=8.5,height=11,onefile = TRUE)
...
#   
  ...
  slist<-list(qdf,pdodf,Umudf,Usigdf,tauRdf,phidf,psidf)
  nm<-c("q","pdocoef","Umu","Usig","tau","phi","psi")
  i=2
