...
q ~ dnorm(0,1);  #Fraction of Spawn Observed on spawn surveys estiamted across all sites

#psi ~ dgamma(1,100) #variance on fishing
...
for(i in 2:nYears) {
 ...
 ",file="normal_spatialRW_11sites_CAR_unequalvar_2015_v3.txt")
...
model.loc="normal_spatialRW_11sites_CAR_unequalvar_2015_v3.txt" # name of the txt file
...
save("model","mDIC","X","U","Umu","Usig","Z","delta","Qinv","phi","pdo","pdocoef","Pc","q","tauR","tau2",file="herring_jags_F_11sites_UNequal_var_CAR_2015_v5.RData")

load("herring_jags_F_11sites_UNequal_var_CAR_2015_v4.RData")
...
#head(model$BUGSoutput$sims.array)
  ...
  pdf(paste("11sites_CAR_UNequalvar_Chains_Posteriors_1950_2015_psi",Sys.Date(),".pdf"), width=8.5,height=11,onefile = TRUE)
...
#   
  ...
  udf$section2<-as.character(sort(rep(unique(x$section),nrow(udf)/nSites)))
  ...
  sdf$section2<-as.character(sort(rep(unique(x$section),nrow(udf)/nSites)))
