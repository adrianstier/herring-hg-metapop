...
for(j in 1:nSites) {
      X[1,j] ~ dnorm(5,inverse(sigma2[j])); # vague normal prior for first time step
...
X[i,j] <-Z[i,j]+log(1-Pc[i,j]);
 ...
 ",file="normal_spatialRW_11sites_DiagUNEqual.txt")
...
model.loc="normal_spatialRW_11sites_DiagUNEqual.txt" # name of the txt file
...
save("model","mDIC","X","sigma2","U","Umu","Usig","Z","delta","sigma2","pdo","pdocoef","Pc","q","tauR",file="herring_jags_F_11sites_diag_UNequal_var.RData")

load("herring_jags_F_11sites_diag_UNequal_var.RData")
...
pdf(paste("11sites_diag_UNequal_var_Chains_Posteriors",Sys.Date(),".pdf"), onefile = TRUE)
...
ucomp<-data.frame("nocovmean"=tapply(udf$value,list(udf$Var3),mean),"nocovsd"=tapply(udf$value,list(udf$Var3),sd))
ucomp$site<-rownames(ucomp)
write.csv(ucomp,"nocovUest.csv")
