...
X[i,j] <-Z[i,j]+log(1-Pc[i,j]);
 ...
 ",file="normal_spatialRW_11sites_DiagEqual.txt")
...
jags.params=c("X","sigma2","U","Umu","Usig","delta","Z","pdocoef","q","Pc","tauR") # parameters in the linear model


model.loc="normal_spatialRW_11sites_DiagEqual.txt" # name of the txt file
...
save("model","mDIC","X","sigma2","U","Umu","Usig","Z","delta","sigma2","pdo","pdocoef","Pc","q","tauR",file="herring_jags_F_11sites_diag_equal_var.RData")

load("herring_jags_F_11sites_diag_equal_var.RData")
########################################################################
...
pdf(paste("Chains and Posteriors_11sites_DiagEqual",Sys.Date(),".pdf"), onefile = TRUE)
...
slist<-list(qdf,pdodf,Umudf,Usigdf,tauRdf,sigma2df)
nm<-c("q","pdocoef","Umu","Usig","tauR","sigma2")
