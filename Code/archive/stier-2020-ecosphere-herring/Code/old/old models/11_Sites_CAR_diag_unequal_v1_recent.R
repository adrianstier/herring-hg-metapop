...
#subset out 2000: 2015 for recent  u estimates
Y_car<-Y_car[c(51:66),]
...
ctab2_1_car<-ctab[,c(9,10,11,8,12,5,1,4,3,2,7,6)] #car model
...
ctab2_1_car<-ctab2_1_car[c(51:66),]
...
save("model","mDIC","X","U","Umu","Usig","Z","delta","Qinv","phi","pdo","pdocoef","Pc","q","tauR","tau2",file="herring_jags_F_11sites_UNequal_var_CAR_recent.RData")

load("herring_jags_F_11sites_UNequal_var_CAR_recent.RData")
########################################################################
...
pdf(paste("11sites_CAR_UNequalvar_Chains_Posteriors_RECENT",Sys.Date(),".pdf"), width=17,onefile = TRUE)
...
nYears=nrow(model$BUGSoutput$mean$X)
...
mms<-scale(Y_car)
mms<-data.frame("year"=seq(2000,2013,1),mms)
...
dfa<-data.frame("year"=seq(2000,2013,1),"cumspawn"=scale(rowSums(Y_car,na.rm=T)))
