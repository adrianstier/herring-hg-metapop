...
sitelab2<-drop.levels(unique(x$section_name)[-c(4,6)])
...
#AS making a mess of the covariance matrix
#########################################################################################################
tau2Inv ~ dgamma(0.01,0.01) #inv variance
phi		~ dunif(PHI.min+1e-6,PHI.max-1e-6) #modifier of cov param

Q <- ((tau2Inv)^(-1))*inverse(IDEN - phi * H)
...
for(i in 2:nYears) {
 ...
 ",file="normal_spatialRW_11sites_CAR_identity.txt")
...
jags.data.p = list("Y", "nYears","nSites","pdo","ctab","c_prior","PHI.min","PHI.max","IDEN","H") # named list



jags.params=c("X","U","Umu","Usig","tau2Inv","phi","delta","Q","Z","pdocoef","q","Pc","tauR") # parameters in the linear model


model.loc="normal_spatialRW_11sites_CAR_identity.txt" # name of the txt file
...
save("model","mDIC","X","U","Umu","Usig","Z","delta","Q","pdo","pdocoef","Pc","q","tauR","tau2Inv",file="herring_jags_F_11sites_equal_var_CAR.RData")

load("herring_jags_F_11sites_equal_var_CAR.RData")
########################################################################
...
pdf(paste("11sites_CAR_equalvar_Chains_Posteriors",Sys.Date(),".pdf"), width=17,height=22,onefile = TRUE)
...
tau2Invdf<-melt(model$BUGSoutput$sims.array[,,"tau2Inv"])
phidf<-melt(model$BUGSoutput$sims.array[,,"phi"])

slist<-list(qdf,pdodf,Umudf,Usigdf,tauRdf,tau2Invdf,phidf)
nm<-c("q","pdocoef","Umu","Usig","tauR","tau2Inv","phi")
...
udf$section2<-as.character(sort(rep(unique(x$section)[-c(4,6)],nrow(udf)/nSites)))
