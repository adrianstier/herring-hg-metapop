...
#VARINCE SPATIAL VARIANCE AND COVARIANCE
  ...
  for(i in 1:nSites) {
    sigma2[i] ~ dgamma(0.001,0.001); #this is for sd in cov matrix
}

for(i in 1:nSites){
 sigma2b[i] <-sqrt(sigma2[i])
}

#eta ~ dgamma(0.01,0.01) #wiggle on distance decay
#logtheta ~ dnorm(0,0.01) #rate of distance decay
#theta <- exp(logtheta)
theta ~ dunif(0.001,0.99)
...
for(i in 1:nSites) {
for(j in 1:nSites) {
    Q[i,j] <- sigma2b[i]*sigma2b[j] * exp(-theta * distMat5[i,j]) #no nugget 
...
delta[1,1:nSites] ~ dmnorm(zeros,tauQ[1:nSites,1:nSites]); #still not sure i understand 1:nSites here 

# change in pop through time that's uniqe to site/time
for(i in 2:nYears) {
delta[i,1:nSites] ~ dmnorm(zeros,tauQ[1:nSites,1:nSites]); 
#delta[i,1:nSites] ~ dmnorm(delta[i-1,1:nSites],tauQ[1:nSites,1:nSites]); #autoregressive
...
X[i,j] <-Z[i,j]+log(1-Pc[i,j]);
 ...
 ",file="normal_DD_11sites_DD_Diag_Unequal.txt")


#data going into the model
jags.data = list("Y"=Y, "nYears"=nYears,"nSites"=nSites,"distMat5"=distMat5,"pdo"=pdo3,"ctab"=ctab2_1,"c_prior"=c_prior) # named list
jags.data.p = list("Y", "nYears","nSites","distMat5","pdo","ctab","c_prior") # named list



jags.params=c("X","theta","sigma2","U","Umu","Usig","delta","Q","Z","pdocoef","q","Pc","tauR") # parameters in the linear model


model.loc="normal_DD_11sites_DD_Diag_Unequal.txt" # name of the txt file
...
save("model","mDIC","X","theta","sigma2","U","Umu","Usig","Z","delta","Q","pdo","pdocoef","Pc","q","tauR",file="herring_jags_F_11sites_DD_unequal_var.RData")

load("/Users/AdrianStierMBP2015/Dropbox/Projects/In Progress/Herring_Haida_Gwaii/Code/old/RData_old/herring_jags_F_11sites_DD_unequal_var.RData")
...
pdf(paste("11sites_DD_Diagonal_UN_Equal_Chains_Posteriors",Sys.Date(),".pdf"), onefile = TRUE)
...
#labels for graph
sitelab<-paste(rep("Section",12),as.character(unique(x$section)[-c(4,7)]))
...
sdf<-melt(model$BUGSoutput$sims.array[,,colnames(myList[[1]])[which(colnames(myList[[1]])=="sigma2[1]"):which(colnames(myList[[1]])=="sigma2[11]")]])
...
slist<-list(qdf,pdodf,thetadf,Umudf,Usigdf,tauRdf)
...
edf1<-melt(model$BUGSoutput$sims.array[,,colnames(myList[[1]])[which(colnames(myList[[1]])=="Q[1,1]"):which(colnames(myList[[1]])=="Q[11,11]")]])
names(edf1)<-c("num","chain","response","value")
edf1$chain<-factor(edf1$chain)
