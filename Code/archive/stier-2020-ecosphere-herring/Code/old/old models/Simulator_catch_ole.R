...
for(k in 1:nIndex){
    ...
    ctab[INDEX[k,1],INDEX[k,2]] ~ dnorm((tl[INDEX[k,1],INDEX[k,2]]),1/psi) #distribution of catch data  LIKELIHOOD STATEMENT ****  
...
for(j in 1:nSites) {
      Z[1,j] ~ dnorm(5,1/big.var); # vague normal prior for first time step #changed from inverse 3/31
...
for(i in 1:nYears){
  for(j in 1:nSites) {delta[i,j] ~ dnorm(0,1/sigma2.all[j]) #changed from inverse 3/31
  }
}
...
jags.data = list("Y"=SHI_sim, "nYears"=nYears,"nSites"=nSites,"pdo"=pdo3,"ctab"=catch_sim,
                 "INDEX"=INDEX, "nIndex"=nIndex,"Catch.dummy"=Catch.dummy) # named list
...
save("model","mDIC","X","sigma2","U","Umu","Usig2","Z","delta","sigma2","pdo","pdocoef","Pc","Pc.mat","q","tauR","psi",
