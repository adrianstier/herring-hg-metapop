...
for(i in 1:nYears) {
  for(j in 1:nSites) {
    ...
    ctab[i,j] ~ dnorm(tl[i,j]q,1/.1) #distribution of catch data  LIKELIHOOD STATEMENT ****
...
X[i,j] <-Z[i,j]+log(1-Pc[i,j]);
 ...
 ",file="normal_spatialRW_11sites_DiagEqual_v2_fixedpsi.txt")
...
model.loc="normal_spatialRW_11sites_DiagEqual_v2_fixedpsi.txt" # name of the txt file
...
save("model","mDIC","X","sigma2","U","Umu","Usig","Z","delta","sigma2","pdo","pdocoef","Pc","q","tauR",file="herring_jags_F_11sites_diag_equal_var_v2.RData")

#js imac 
setwd("~/Dropbox/Herring stocklet dynamics/Code")
...
pdf(paste("Chains and Posteriors_11sites_DiagEqual_v2",Sys.Date(),".pdf"), onefile = TRUE)
