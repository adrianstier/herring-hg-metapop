...
## ==================
##  Population Growth - single umu (non hierarchical)
## ==================

Umu_sim  <- 0.1 #mean(umat[,1])
...
for(i in 1:nYears){
  for(j in 1:nSites){	
    Pc_sim[i,j]<-ifelse(logcatch[i,j]>0,
                        runif(1,0.001,0.4), #turn up/down fishing here
                        0
                        )
  }
}
...
for(j in 1:nSites){
  for(i in 2:nYears){
    zmat[i,j]     <- xmat[i-1,j] + Umu_sim + delta_sim[i-1,j] + pdocoef_sim*pdo2[i] 
...
SHI_sim <- rbind(xmat[1:38,]+log_q_sim_1, xmat[39:66,]+log_q_sim_2)
SHI_obs <- matrix(rnorm(length(SHI_sim),SHI_sim,sqrt(tauR2_sim)),nrow(SHI_sim),ncol(SHI_sim)) #add error in observation (tauR2_sim)
...
model {  
 ...
 ",file="simulator_diagonal_equal_var_2q.txt")


#data going into the model
jags.data = list("Y"=SHI_obs, "nYears"=nYears,"nSites"=nSites,"pdo"=pdo2,"ctab"=catch_sim,
...
model.loc="simulator_diagonal_equal_var_2q.txt" # name of the txt file
...
save("model","mDIC","X","sigma2","Umu","Z","delta","sigma2","pdo","pdocoef","Pc","log.q","tauR2",#"psi",
     "log_q_sim_1","log_q_sim_2","pdocoef_sim","Umu_sim","tauR2_sim","sigma2_sim","design.c",
     file="simulator_diag_equal_design.c_2q.RData")
...
load("simulator_diag_equal_design.c_2q.RData")
########################################################################
...
pdf(paste("simulator_diagonal_equal_design_c_2q",Sys.Date(),".pdf"), onefile = TRUE)
...
y2<-data.frame("time"=seq(1,nYears,1),SHI_sim)
...
umuplot<-ggplot(umudf2,aes(x=ttt,y=Median,ymin=Lower,ymax=Upper))+
  ...
  geom_hline(yintercept=Umu_sim,col=2)
...
simvec<-c(log_q_sim_1,log_q_sim_2,tauR2_sim,pdocoef_sim,Umu_sim,sigma2_sim[1])#,psi_sim)
