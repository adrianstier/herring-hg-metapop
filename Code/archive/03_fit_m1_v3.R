# ============================================================================
# 03_fit_m1_v3.R — Fit Model M1 v3 (Improved baseline, site-specific sigma_obs)
...
cat("\n", strrep("=", 60), "\n")
cat(" MODEL M1 v3: Improved Baseline (Okamoto-inspired)\n")
cat(strrep("=", 60), "\n\n")
...
stan_file <- here("inst", "stan", "herring_metapop_m1_v3.stan")
...
saveRDS(fit, file.path(data_dir, "m1_v3_fit.rds"))
saveRDS(fit, file.path(out_dir, "fit_m1_v3.rds"))

# LOO
log_lik <- rstan::extract(fit, "log_lik", permuted = FALSE)
r_eff <- relative_eff(exp(log_lik))
loo_m1 <- loo(log_lik, r_eff = r_eff)
saveRDS(loo_m1, file.path(out_dir, "loo_m1_v3.rds"))

# Parameter summary
summ <- summarize_draws(fit)
write.csv(summ, file.path(proj_dir, "Output", "m1_v3_parameter_summary.csv"), row.names = FALSE)
