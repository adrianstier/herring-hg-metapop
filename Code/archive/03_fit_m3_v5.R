# ============================================================================
# 03_fit_m3_v5.R — Fit Model M3 v5 (DD + lagged timing / habitat covariates)
...
cat("\n", strrep("=", 60), "\n")
cat(" MODEL M3 v5: DD + lagged timing / habitat covariates\n")
cat(strrep("=", 60), "\n\n")
...
# ============================================================================
# 3. LOAD PROCESS COVARIATES
# ============================================================================

cov_path <- file.path(data_dir, "dfo_spawn_covariates_region_1951_2025.csv")
cov_df <- read_csv(cov_path, show_col_types = FALSE) %>%
  select(year, weighted_spawn_start_doy, subtidal_share) %>%
...
cov_df <- cov_df %>%
  mutate(
    weighted_spawn_start_doy_z = zscore(weighted_spawn_start_doy),
    subtidal_share_z = zscore(subtidal_share)
  )

spawn_cov <- as.matrix(cov_df[, c("weighted_spawn_start_doy_z", "subtidal_share_z")])
...
stopifnot(
  ...
  identical(cov_df$year, jags_data$years),
  all(is.finite(stan_data$spawn_cov)),
...
make_init_m3_v3 <- function() {
  list(
    ...
    nu_obs = 5,
    beta_cov = rep(0, stan_data$K_cov),
    log_q = c(-1.2, -0.2, 0.0),
...
stan_file <- here("inst", "stan", "herring_metapop_m3_v5.stan")
...
saveRDS(fit, file.path(data_dir, "m3_v5_fit.rds"))
saveRDS(fit, file.path(out_dir, "fit_m3_v5.rds"))

# LOO
log_lik <- rstan::extract(fit, "log_lik", permuted = FALSE)
r_eff <- relative_eff(exp(log_lik))
loo_m3 <- loo(log_lik, r_eff = r_eff)
saveRDS(loo_m3, file.path(out_dir, "loo_m3_v5.rds"))

# Parameter summary
summ <- summarize_draws(fit)
write.csv(summ, file.path(proj_dir, "Output", "m3_v5_parameter_summary.csv"), row.names = FALSE)
