# ============================================================================
# 03_fit_m3_v2.R — Fit Model M3 v2 (Gompertz DD, Student-t, censored zeros)
...
cat("\n", strrep("=", 60), "\n")
cat(" MODEL M3 v2: Gompertz DD + Student-t + Censored zeros\n")
cat(strrep("=", 60), "\n\n")
...
stan_file <- here("inst", "stan", "herring_metapop_m3_v2.stan")
...
saveRDS(fit, file.path(data_dir, "m3_v2_fit.rds"))
saveRDS(fit, file.path(out_dir, "fit_m3_v2.rds"))

# LOO
log_lik <- extract_log_lik(fit)
r_eff <- relative_eff(exp(log_lik))
loo_m3 <- loo(log_lik, r_eff = r_eff)
saveRDS(loo_m3, file.path(out_dir, "loo_m3_v2.rds"))

# Parameter summary
summ <- summarize_draws(fit)
write.csv(summ, file.path(proj_dir, "Output", "m3_v2_parameter_summary.csv"), row.names = FALSE)
