# ============================================================================
# 03_fit_m1_v5.R — Fit Model M1 v5 (method-specific positive scale)
...
cat("\n", strrep("=", 60), "\n")
cat(" MODEL M1 v5: Stable baseline + method-specific positive scale\n")
cat(strrep("=", 60), "\n\n")
...
make_init_m1_v5 <- function() {
  list(
    ...
    beta_det = 1.0,
    log_q = c(-1.2, -0.2, 0.0),
    log_sigma_obs_method = c(log(1.25), log(1.05), 0.0),
...
stan_file <- here("inst", "stan", "herring_metapop_m1_v5.stan")
...
saveRDS(fit, file.path(data_dir, "m1_v5_fit.rds"))
saveRDS(fit, file.path(out_dir, "fit_m1_v5.rds"))
...
saveRDS(loo_m1_raw, file.path(out_dir, "loo_m1_v5_raw_psis.rds"))
...
saveRDS(loo_m1, file.path(out_dir, "loo_m1_v5.rds"))

# Parameter summary
summ <- summarize_draws(fit)
write.csv(summ, file.path(proj_dir, "Output", "m1_v5_parameter_summary.csv"), row.names = FALSE)
