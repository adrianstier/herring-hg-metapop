# ============================================================================
# 03_fit_m5_v5.R — Fit Model M5 v5 (DD + predators + lagged DFO covariates)
...
cat("\n", strrep("=", 60), "\n")
cat(" MODEL M5 v5: DD + predators + lagged DFO covariates\n")
cat(strrep("=", 60), "\n\n")
...
stan_file <- here("inst", "stan", "herring_metapop_m5_v5.stan")
...
saveRDS(fit, file.path(data_dir, "m5_v5_fit.rds"))
saveRDS(fit, file.path(out_dir, "fit_m5_v5.rds"))

# LOO
log_lik <- rstan::extract(fit, "log_lik", permuted = FALSE)
r_eff <- relative_eff(exp(log_lik))
loo_m5 <- loo(log_lik, r_eff = r_eff)
saveRDS(loo_m5, file.path(out_dir, "loo_m5_v5.rds"))

# Parameter summary
summ <- summarize_draws(fit)
write.csv(summ, file.path(proj_dir, "Output", "m5_v5_parameter_summary.csv"), row.names = FALSE)
