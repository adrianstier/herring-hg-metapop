# ============================================================================
# 03_fit_m5_stier_predation_pressure.R
# Fit Stier-aligned M5 with HG predation-pressure covariate.
#
# Key choices:
#   - zero spawn records remain ambiguous / skipped,
#   - survey method uses the original Stier two-era split,
#   - predator effect enters as lagged annual HG predation pressure,
...
cat("\n", strrep("=", 60), "\n")
cat(" MODEL M5_STIER_PREDATION_PRESSURE: Stier obs + HG predator pressure\n")
cat(strrep("=", 60), "\n\n")
...
pred_pressure <- pred_covariates %>%
  select(year, pred_pressure_log_z) %>%
  right_join(tibble(year = jags_data$years), by = "year") %>%
  arrange(year) %>%
  pull(pred_pressure_log_z)
...
stopifnot(
  ...
  all(is.finite(stan_data$pred_pressure)),
  all(is.finite(stan_data$log_catch)),
  all(is.finite(stan_data$Y[stan_data$Y_obs_flag == 1L]))
)

cat("Stier-aligned predator-pressure data choices:\n")
...
cat("  Predator covariate range:", paste(round(range(stan_data$pred_pressure), 3), collapse = " to "), "\n")
cat("  Fitted sections:", stan_data$N_sites, "\n\n")
print_fit_control(fit_control)

make_init_m5_stier_predation_pressure <- function() {
  list(
    Umu = 0,
    pdocoef = 0,
    predcoef = 0,
...
stan_file <- here("inst", "stan", "herring_metapop_m5_stier_predation_pressure.stan")
fit <- stan(
  file = stan_file,
  data = stan_data,
  ...
  refresh = 100,
  init = make_init_m5_stier_predation_pressure,
...
saveRDS(fit, file.path(data_dir, paste0("m5_stier_predation_pressure", artifact_suffix, "_fit.rds")))
saveRDS(fit, file.path(out_dir, paste0("fit_m5_stier_predation_pressure", artifact_suffix, ".rds")))

if (!fit_control$skip_postfit) {
  log_lik <- rstan::extract(fit, "log_lik", permuted = FALSE)
  r_eff <- relative_eff(exp(log_lik))
  loo_m5 <- loo(log_lik, r_eff = r_eff)
  saveRDS(loo_m5, file.path(out_dir, "loo_m5_stier_predation_pressure.rds"))
} else {
...
write.csv(
  ...
  file.path(
    proj_dir,
    "Output",
    paste0("m5_stier_predation_pressure", artifact_suffix, "_parameter_summary.csv")
  ),
  row.names = FALSE
)

cat("\n=== M5_STIER_PREDATION_PRESSURE COMPLETE ===\n")
