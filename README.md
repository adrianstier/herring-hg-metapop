...
This table is the original family-level roadmap. It is not the current
promotion status table. For current model-use classes, read
`Output/diagnostics/model_decision_ledger.md` and
`docs/talk-model-claim-control-sheet.md`: `m1_stier_11` is promoted; fitted
non-baseline branches are held or archived.
...
- **Label Doherty-style proxies explicitly.** Public HG DFO tables are the first source for catch, age composition, weight-at-age, maturity, and current assessment context. WCVI/Doherty values can be shown only as provisional analogues for missing length/size/selectivity or model-structure pieces. They are not Haida Gwaii-estimated catch-at-age, size-at-age, or predator-selectivity parameters. The proxy ledger is `Output/diagnostics/doherty_proxy_parameter_plan.md`, regenerated by `Code/07bo_doherty_proxy_parameter_plan.R`; the concise replication gap table is `docs/doherty-style-hg-gap-table.md`.
...
9. keep `m5_stier_predation_pressure` and `m5_stier_predator_demand_total` as
   held predator screens: both keep the Stier-aligned observation layer, but
   neither materially improves calibration over `m1_stier_11`;
10. use `Output/diagnostics/postclosure_recovery_mechanism_screen.md` as the
    current gate for "why no full recovery after closure" before adding another
    predator, timing/substrate, or interaction Stan branch.
...
The post-closure recovery mechanism gate is
`Output/diagnostics/postclosure_recovery_mechanism_screen.md` with figure
`Output/figures/postclosure_recovery_mechanism_screen.pdf`. It asks why closure
alone did not restore all local states by screening legacy fishing, raw
spawn-location persistence, predator/climate pressure, timing/substrate context,
and evidence-quality caveats. The current read is conservative: no
post-closure section-year predator/climate row clears the strict gate; legacy
depletion and unresolved local mechanisms remain the most defensible talk
message.
The lag-specific companion is
`Output/diagnostics/future_lag_negative_control_audit.md` with figure
`Output/figures/future_lag_negative_control_audit.pdf`. It keeps immediate
adult predation separate from delayed age-3 recruitment-return hypotheses.
Current read: adult lag-1 and biomass-growth age-3 rows do not clear the gate;
public Appendix B number-at-age rows are provisional age-composition audit
targets, while spawn-normalized rows are shared-input context and the DFO 2025
age-2 recruitment summary is SCA model-output context only.
...
Key May 9 diagnostics so far: recent biomass is concentrated in a few sections, three focal sections remain below 20% of their 1951-1965 section baseline in the recent closure period, historical fishing pressure is a strong but incomplete section-level driver, and the closure-response diagnostic shows why fishing history and recovery must be separated: recent biomass is about 1.52x the roe-fishery median after fishing ended, but median occupied sections fall from 8 during the roe fishery to 5 recently. Cumshewa and Louscoone are the clearest depletion-beyond-fishing cases. The post-closure mechanism screen now adds the direct answer to "why no full recovery after no fishing": no predator/climate section-year row clears the gate, while endpoint context points back to legacy depletion, local spawning-site persistence, and unresolved section-specific mechanisms. The future-lag audit adds an important timing caveat: if predation works through egg/juvenile survival, an age-3 return lag is biologically plausible, but the current biomass-growth proxy remains weak; Appendix B number-at-age is provisional, spawn-normalized rows share the model's spawn input, and DFO age-2 recruitment is SCA output, so those rows are not independent model-ready responses. The existing lagged-PDO baseline effect is negative but uncertain; a cheap PDO window screen finds lag 0-1 slightly stronger while lag 1 remains competitive, so do not launch a redundant PDO-only branch. Predator indices are strongly time-confounded, density-dependence evidence is weak, and residual spawn-fit correlations show only weak distance decay. The new predator spatial exposure prototype makes the next predator step clearer: refine section-level exposure from raw seal/sea-lion locations before fitting a predator coefficient. Those results keep the near-term priority on section heterogeneity, local exposure data products, and observation calibration before predator or age/size model branches.
...
analysis. Start from `docs/talk-model-claim-control-sheet.md` and
`docs/doherty-style-hg-gap-table.md`. Use
