// ============================================================================
// herring_metapop_m1_v5.stan — M1v5: Stable baseline + method-specific
// positive-observation scale
//
// IMPROVEMENTS from v4:
//   1. Keeps the sampler-stable M1 baseline process and detection structure.
//   2. Adds method-specific positive-observation scale multipliers.
//   3. Avoids a separate positive-observation bias term so `log_q` remains
//      interpretable as the method-specific survey scale.
//   4. Keeps the threshold-aware truncated positive likelihood.
// ============================================================================
...
parameters {
  ...
  vector[N_methods] log_sigma_obs_method;
...
model {
  ...
  log_sigma_obs_method[1] ~ normal(0, 0.35);
  log_sigma_obs_method[2] ~ normal(0, 0.20);
  log_sigma_obs_method[3] ~ normal(0, 0.15);
  ...
  if (prior_only == 0) {
    for (t in 1:N_years) {
      for (j in 1:N_sites) {
        real mu_det = X[t, j] + log_q[q_idx[t]];
        real sigma_obs_eff = sigma_obs[j] * exp(log_sigma_obs_method[q_idx[t]]);
        real p_detect = inv_logit(alpha_det[q_idx[t]] + beta_det * (mu_det - log_detection_threshold));
        if (Y_obs_flag[t, j] == 1) {
          target += bernoulli_lpmf(1 | p_detect);
          target += student_t_lpdf(Y[t, j] | nu_obs, mu_det, sigma_obs_eff) -
            student_t_lccdf(log_detection_threshold | nu_obs, mu_det, sigma_obs_eff);
        }
      ...
      for (j in 1:N_sites) {
        ...
        if (Y_obs_flag[t, j] == 1) {
          ...
          log_lik[idx] = bernoulli_lpmf(1 | p_detect) +
            student_t_lpdf(Y[t, j] | nu_obs, mu_det, sigma_obs_eff) -
            student_t_lccdf(log_detection_threshold | nu_obs, mu_det, sigma_obs_eff);
        }
        if (Y_censored_flag[t, j] == 1) {
          idx += 1;
        ...
        if (detected == 1) {
          real y_rep_log = student_t_rng(nu_obs, mu_det, sigma_obs_eff);
          while (y_rep_log <= log_detection_threshold) {
            y_rep_log = student_t_rng(nu_obs, mu_det, sigma_obs_eff);
          }
