// ============================================================================
// herring_metapop_m3_v4.stan — Okamoto-inspired Metapopulation Model v4
//
// IMPROVEMENTS from v3:
//   1. Informative q prior (Martell et al. 2012): Normal(-0.569, 0.274) for dive
//      (Median = ~0.566 on natural scale). This assumes dive surveys see ~57% of eggs.
// ============================================================================
...
transformed parameters {
  ...
  for (t in 2:N_years) {
    for (j in 1:N_sites) {
      Z[t, j] = (1 + b[j]) * X[t - 1, j] + U[j] + pdocoef * pdo[t - 1] + epsilon[t - 1, j];
...
model {
  ...
  // Informative q prior (Martell 2012)
  log_q[3] ~ normal(-0.569, 0.274); // Dive
  log_q[1] ~ normal(0, 2.0);        // Surface
  log_q[2] ~ normal(0, 2.0);        // Mixed
...

  {
    ...
    for (t in 1:N_years) {
      for (j in 1:N_sites) {
        if (Y_obs_flag[t, j] == 1) {
          idx += 1;
          log_lik[idx] = student_t_lpdf(Y[t, j] | nu_obs, X[t, j] + log_q[q_idx[t]], sigma_obs[j]);
          Y_rep[t, j] = exp(student_t_rng(nu_obs, X[t, j] + log_q[q_idx[t]], sigma_obs[j]));
        } else {
          Y_rep[t, j] = 0.0;
        }
