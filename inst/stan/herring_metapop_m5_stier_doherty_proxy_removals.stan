// ============================================================================
// herring_metapop_m5_stier_doherty_proxy_removals.stan
//
// HG Doherty-style proxy-removal branch:
//   1. Keeps the 11-section positive-only, zeros-ambiguous Stier observation
//      layer.
//   2. Keeps the two-era surface/SCUBA q split.
//   3. Treats audited annual HG predator demand as a fixed natural-mortality
//      offset using the predator repo's Doherty-style Mp proxy.
//   4. This is not an age-structured catch-at-age model and does not use
//      predator selectivity-at-age. It is a biomass-scale proxy for the talk
//      and AWS troubleshooting loop.
// ============================================================================
...
data {
  ...
  vector[N_years] pdo;
  vector<lower=0>[N_years] pred_mortality;
  real<lower=0> pred_scale_fixed;
...
transformed parameters {
  real<lower=0> pred_scale = pred_scale_fixed;
  matrix[N_years, N_sites] Z;
  matrix[N_years, N_sites] X;
  matrix[N_years, N_sites] Pc_mat;
  vector<lower=0>[N_years] pred_mortality_effect;
  ...
  pred_mortality_effect = pred_scale * pred_mortality;
