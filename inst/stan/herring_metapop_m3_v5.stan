// ============================================================================
// herring_metapop_m3_v5.stan — DD + lagged timing / habitat covariates
//
...
//   5. Lagged regional timing / habitat covariates with regularizing priors
// ============================================================================
...
transformed data {
  ...
  for (t in 1:N_years) {
    for (j in 1:N_sites) {
      if (Y_obs_flag[t, j] == 1 || Y_censored_flag[t, j] == 1) {
        N_surveyed += 1;
      }
    }
  }
...
model {
  ...
  // For Dive (idx 3): tight Normal(0, 0.05)
  // For Surface (idx 1): vague Normal(0, 2)
  // For Mixed (idx 2): vague Normal(0, 2)
...

  {
    ...
    for (t in 1:N_years) {
      for (j in 1:N_sites) {
        ...
        if (Y_censored_flag[t, j] == 1) {
          idx += 1;
          log_lik[idx] = student_t_lcdf(log_detection_threshold | nu_obs, mu, sigma_obs[j]);
        }
