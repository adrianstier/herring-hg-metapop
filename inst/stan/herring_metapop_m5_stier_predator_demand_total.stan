// ============================================================================
// herring_metapop_m5_stier_predator_demand_total.stan
//
// Stier-aligned predator-demand branch:
//   1. Keeps the 11-section positive-only, zeros-ambiguous observation model.
//   2. Keeps the two-era surface/SCUBA q split.
//   3. Adds lagged total HG predator demand, z(log1p(C_total_kt)).
//   4. Uses predator demand rather than pressure_pct because pressure_pct
//      includes observed herring spawn in its denominator.
// ============================================================================
...

  {
    ...
    for (t in 1:N_years) {
      for (j in 1:N_sites) {
        ...
        pred_effect_total[t, j] = predcoef * pred_demand[t];
