// ============================================================================
// herring_metapop_m2_stier_site_growth.stan
//
// Stier-aligned section-heterogeneity branch:
//   1. Same positive-only, zeros-ambiguous observation model as m1_stier_11.
//   2. Same two survey-era q terms: surface and SCUBA/dive.
//   3. Adds hierarchical section-specific productivity U[j].
//   4. Does not add predators, density dependence, age/size, or spatial
//      correlation yet.
//
// Purpose: test whether the major section winners/losers are better explained
// by persistent section differences before adding regional driver covariates.
// ============================================================================
...
model {
  U_mu ~ normal(0, 1);
  sigma_U ~ normal(0, 0.25);
  U_raw ~ std_normal();

  pdocoef ~ normal(0, 1);
  sigma_proc ~ student_t(3, 0, 2.5);

  // Same tonnes-scale observation model as m1_stier_11.
