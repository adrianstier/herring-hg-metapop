...
REPO = Path("/sessions/festive-gracious-bell/mnt/stier-2027-herring-metapopulation")
...
rcParams.update({
    "font.family": "Liberation Serif", "text.color": INK,
...
def sb2_predator_species():
    ...
    ax.set_xlabel("Mean recent annual herring demand (kt · yr⁻¹)",
...
def sb6_cod_vs_herring():
    ...
    # Herring — falls, fishery closes, stays low
    herr = 1.0 - 0.85 * np.exp(-((t - 0.35) ** 2) / 0.012) - 0.05 * (t > 0.5)
    herr = np.clip(herr, 0.08, 1.0)
    # add slight noise rebound that fades
    herr = herr + 0.03 * np.sin((t - 0.5) * 18) * (t > 0.5)
